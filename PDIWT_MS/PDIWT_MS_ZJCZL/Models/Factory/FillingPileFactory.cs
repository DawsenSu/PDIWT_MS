...
{
    class FillingPileFactory : PileFactory
    {
        public override PileBase CreateNewPile(IPileProperty pileType, string pilecode, long pileId)
        {
            ...
            var tempLayerBase = new ObservableCollection<FillingPileSoilLayerInfo>();
            ...
            for (int i = 0; i < resultlayer.Count; i++)
            {
                ...
                tempLayerBase.Add(new FillingPileSoilLayerInfo() { SoilLayerName = resultlayer[i].IntersectLayerInfo.Category, SoilLayerNum = resultlayer[i].IntersectLayerInfo.UserCode, PileInSoilLayerLength = temppileLength, Qfi = random.Next(), Xii = random.Next(),PsiSi = random.Next()});
            }
            return new FillingPile() { PileCode = pilecode, PileId = pileId, PilePropertyInfo = pileType, FillingPileSoilLayerInfoProp = tempLayerBase, GammaR = 1.3, Qr = 6000, PsiP=800 };
        }
    }
}
